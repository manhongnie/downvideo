...
class Runtime:
    def __init__(self, database: str | Path | None = None, on_event: EventSink = lambda event: None,
        ...
        self._http = HttpPageFetcher(proxy=proxy)
        self._browser: OptionalBrowserFetcher | None = None
        self._resolver = HybridVideoResolver(proxy=proxy)
        ...
        self.downloads = DownloadService(YtDlpDownloader(proxy=proxy), self.repository, self._emit,
        ...
        if config.browser:
            if self._browser is None:
                self._browser = OptionalBrowserFetcher(self._http,
                    warning=lambda message: self._emit(Event("log", message=message)), proxy=self.proxy)
...
def build_runtime(database: str | Path | None = None, on_event: EventSink = lambda event: None,
                  *, proxy: str | None = None) -> Runtime:
    return Runtime(database, on_event, proxy=proxy)
