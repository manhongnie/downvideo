...
```bash
conda run -n downvideo python -m pip install 'playwright>=1.50,<2'
conda run -n downvideo playwright install chromium
```

在高级设置勾选浏览器增强。遇到 Cloudflare 验证的网站还需勾选“可见浏览器”；扫描时桌面会弹出独立的 Chromium 窗口，窗口关闭后不会保留登录状态。可调整“浏览器操作时间预算”给人工验证留出时间。未安装 Playwright 或 Chromium 会显示明确提示并继续 HTTP 基础扫描。普通 HTML 分页无需浏览器。

也可以用 `video-scout --browser-visible --url 'https://example.com/'` 预先勾选两个选项，再按 F5 扫描。
...
6. 扫描完全结束后「下载选中项」。任务创建时固定目标目录，之后改输入框不改变旧任务。下载任务页可重试当前失败/取消任务，或一键重试全部失败任务；批量重试沿用原任务目录和续传文件。也可取消当前/全部任务。目录自动创建，支持中文和空格。
